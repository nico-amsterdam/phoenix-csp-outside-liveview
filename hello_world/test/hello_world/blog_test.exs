defmodule HelloWorld.BlogTest do
  use HelloWorld.DataCase

  alias HelloWorld.Blog

  describe "posts" do
    alias HelloWorld.Blog.Post
    ...
    test "create_post/1 with valid data creates a post" do
      valid_attrs = %{title: "some title", views: 42}
  ...
  describe "posts" do
    ...
    test "create_post/1 with valid data creates a post" do
      valid_attrs = %{country: "some country", title: "some title", views: 42}

      assert {:ok, %Post{} = post} = Blog.create_post(valid_attrs)
      assert post.country == "some country"
      assert post.title == "some title"
      assert post.views == 42
    end
    ...
    test "update_post/2 with valid data updates the post" do
      ...
      assert {:ok, %Post{} = post} = Blog.update_post(post, update_attrs)
      assert post.country == "some updated country"
      assert post.title == "some updated title"
      assert post.views == 43
    end
